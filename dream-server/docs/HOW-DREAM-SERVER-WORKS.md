...
Sitting between your chat interface and the brain is a service called **LiteLLM**. Think of LiteLLM as a very smart switchboard operator. When your question comes in, LiteLLM decides where to route it. Most of the time, it goes straight to your local AI. But if you want, you can configure it so that certain kinds of questions go to a cloud model instead — Claude, GPT-4, MiniMax, or any of the hundred-plus models available through OpenRouter — while everything else stays local. Or you can tell it: local is always first, but if local is busy or struggling, fall back to the cloud automatically. This hybrid approach gives you flexibility without forcing you to choose all-or-nothing.
...
**Docling** is the document preprocessing pipeline. Before documents reach Qdrant, Docling reads them — PDFs, DOCX, PPTX, HTML, images — and converts them into clean, structured markdown. Where a raw PDF might preserve layout chaos and scanned text artifacts, Docling extracts clean text with headings, tables, and structure preserved. The better the input quality, the better the embeddings, and the better your RAG answers.

**Vaultwarden** is your team's shared password manager. Every Dream Server deployment involves dozens of credentials — API keys, admin tokens, database passwords. Vaultwarden stores all of them in a self-hosted vault that's accessible from any browser or Bitwarden app. You share credentials with your team without sending them over Slack or email. It's not a Dream Server feature per se — it's infrastructure that makes the whole deployment safer to operate.

**Forgejo** is your self-hosted Git server. Dream Server manages a lot of configuration — Caddyfiles, Prometheus rules, n8n workflow exports, prompt templates, Baserow schemas. Forgejo gives all of that configuration a version-controlled home. You track changes, review diffs, and roll back if something breaks. For teams, it's where the ops runbook lives alongside the actual configs.
...
**Hermes Agent** is the autonomous agent. Here's how to think about the difference between the chat interface and an autonomous agent. In the chat, you ask one question at a time and the AI responds. With Hermes, you give the AI a goal — "research this topic, find the three best sources, write me a summary, and save it to a file" — and it figures out the steps on its own, uses tools, browses the web, takes actions, and comes back to you when the task is done. It's the difference between asking someone a question and delegating a project.
...
That's the team. And it keeps growing. Dream Server now has a full observability layer — a suite of tools that watch everything the team is doing so you can understand performance, catch problems early, and make better decisions about how the system runs.

**Prometheus** is the collector. It reaches into every service at regular intervals — every 15 seconds — and asks what its numbers look like. How many requests is the language model handling? What's the error rate? How much memory is each container using? It stores all of these time-series measurements in a database so you can see trends over time, not just a snapshot of right now.

**Grafana** is the visualization layer. It takes the data Prometheus has collected and turns it into charts and dashboards you can read at a glance. Request rates, latency percentiles, token usage broken down by model, GPU utilization, container CPU and memory — all in real time. It also holds the alerting rules: when something crosses a threshold (GPU memory above 90%, disk over 90% full, llama-server unreachable for more than 2 minutes), Grafana fires an alert so you know before it becomes a crisis.

**Langfuse** is the LLM-specific observability tool. While Prometheus measures container-level metrics, Langfuse measures what matters at the model layer: which prompts are being sent, what completions came back, how long each call took, how many tokens were used, and what errors occurred. It traces every LLM request end-to-end. When you're running multiple providers — local models, Claude, GPT-4, MiniMax, OpenRouter — Langfuse shows you which one is actually performing best for your workload. And it integrates with n8n, so when a prompt changes, your automation pipeline can react automatically.

**Uptime Kuma** keeps the history that the other tools don't. It's a dedicated uptime monitor — it checks your services continuously and records whether they were up or down, and for how long. If you need to prove SLA compliance, or just want to know whether a service had a rough night, Uptime Kuma has the record. It sends notifications when something goes down, routing alerts into n8n so you can build workflows around them.

**cAdvisor** and **Node Exporter** are the metrics agents that feed Prometheus. cAdvisor sits inside the Docker layer and breaks down metrics per container — not just aggregate system memory, but how much each specific service is using. Node Exporter reaches down to the host operating system and collects hardware-level data: CPU load, disk I/O, network throughput, filesystem capacity, even GPU temperature on supported hardware.

That completes the team. Somewhere between nineteen and twenty-three people depending on which optional services you've turned on. And they all work together, all day, without you having to think about any of it.
