...
echo "â•‘   Port & Ollama Detection Test Suite     â•‘"
...
source <(sed -n '/^check_port_conflict\s*()\s*{/,/^}/p;/^check_ollama_conflict\s*()\s*{/,/^}/p' \
  "$ROOT_DIR/installers/phases/04-requirements.sh") || {
    echo -e "${RED}âœ— FAIL${NC} - Cannot load functions from 04-requirements.sh"
    exit 1
}
...
if [[ -n "$PORT_CONFLICT_PROC" ]] && [[ "$PORT_CONFLICT_PROC" != "unknown" ]]; then
    ...
    echo -e "${YELLOW}âš  SKIP${NC} (may require lsof)"
fi
...
if [[ "$OLLAMA_RUNNING" == "true" ]]; then
    ...
    echo -e "${YELLOW}âš  SKIP${NC} (Ollama not running)"
fi
