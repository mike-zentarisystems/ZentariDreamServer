#!/bin/bash
# Dream Server â€” Progress Bar Utilities
...
complete_progress_bar() {
    ...
    local width=${2:-40}
    local bar=""
    for ((i=0; i<width; i++)); do bar+="â–ˆ"; done
...
print_phase() {
    ...
    echo -e "\n${BOLD}${BLUE}â–¶ $desc${NC} ${CYAN}(~$duration)${NC}"
}
...
docker_pull_with_progress() {
    ...
    if [[ -t 1 ]]; then
        # TTY available â€” let Docker show native progress
        docker compose -f "$compose_file" pull
    else
        # No TTY â€” use spinner
